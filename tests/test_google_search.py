...
@pytest.fixture
def google_search(mock_google_search, sample_results):
    ...
    source = GoogleSearch()
    mock_google_search.side_effect = side_effect

    return source


class TestGoogleSearch:
    def _test_initialization(self, google_search):
    ...
    @pytest.mark.asyncio
    async def test_invalid_query_raises_error(self, google_search):
        """Test that empty queries raise ValueError"""
        with pytest.raises(ValueError, match="Search query cannot be empty"):
            await google_search._search("")

    @pytest.mark.asyncio
    async def test_search_error_handling(self, google_search, mock_google_search):
        """Test handling of search errors"""
        mock_google_search.side_effect = Exception("API Error")
        with pytest.raises(Exception, match="API Error"):
            await google_search._search("test")

    @pytest.mark.asyncio
    async def test_search_returns_results(self, google_search, sample_query):
        """Test that search returns expected results format"""
        results = await google_search._search(sample_query)
